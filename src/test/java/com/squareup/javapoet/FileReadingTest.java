...
public class FileReadingTest {
    ...
    public void javaFileObjectCharacterContent() throws IOException {
        TypeSpec type = TypeSpec.classBuilder("Test").addJavadoc("Testing, 1, 2, 3!")
                .addMethod(MethodSpec.methodBuilder("fooBar").build()).build();
    ...
    @Test
    public void javaFileObjectInputStream() throws IOException {
        JavaFile javaFile = JavaFile.builder("foo", TypeSpec.classBuilder("Test").build())
                .addFileComment("\u00A9 Copyright character gets you everytime").build();
        byte[] bytes = ByteStreams.toByteArray(javaFile.toJavaFileObject().openInputStream());

        // Be explicit with the default charset to assert the implicit behavior
        // in the code
    ...
    public void compileJavaFile() throws Exception {
        ...
        TypeSpec type = TypeSpec.classBuilder("Test").addModifiers(OpenModifier.PUBLIC)
                .addSuperinterface(ParameterizedTypeName.get(Callable.class, String.class))
                .addMethod(MethodSpec.methodBuilder("call").returns(String.class).addModifiers(OpenModifier.PUBLIC)
                        .addStatement("return $S", value).build())
                .build();
        JavaFile javaFile = JavaFile.builder("foo", type).build();
        ...
        fileManager.setLocation(StandardLocation.CLASS_OUTPUT, Collections.singleton(this.temporaryFolder.newFolder()));
        CompilationTask task = compiler.getTask(null, fileManager, diagnosticCollector, Collections.<String> emptySet(),
                Collections.<String> emptySet(), Collections.singleton(javaFile.toJavaFileObject()));
