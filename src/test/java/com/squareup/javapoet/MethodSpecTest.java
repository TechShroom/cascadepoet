...
public final class MethodSpecTest {
    ...
    public void setUp() {
        ...
        this.types = this.compilation.getTypes();
    }

    private TypeElement getElement(Class<?> clazz) {
        return this.elements.getTypeElement(clazz.getCanonicalName());
    }
    ...
    abstract static class Everything {
        ...
        @Deprecated
        protected abstract <T extends Runnable & Closeable> Runnable
                everything(@Nullable String thing, List<? extends T> things) throws IOException, SecurityException;
    }
    ...
    public void overrideDoesNotCopyDefaultModifier() {
        ...
        List<ExecutableElement> methods = methodsIn(this.elements.getAllMembers(classElement));
        ExecutableElement exec = findFirst(methods, "iterator");
        exec = findFirst(methods, "spliterator");
        MethodSpec method = MethodSpec.overriding(exec, classType, this.types).build();
        assertThat(method.toString()).isEqualTo("" + "@java.lang.Override\n"
    ...
    public void overrideExtendsOthersWorksWithActualTypeParameters() {
        ...
        method = MethodSpec.overriding(exec, classType, this.types).build();
        assertThat(method.toString())
    ...
    public void duplicateExceptionsIgnored() {
        ...
        MethodSpec methodSpec = MethodSpec.methodBuilder("duplicateExceptions").addException(ioException)
                .addException(timeoutException).addException(timeoutException).addException(ioException).build();
