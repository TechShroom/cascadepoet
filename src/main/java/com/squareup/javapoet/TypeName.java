...
/**
 * Any type in Java's type system, plus {@code void}. This class is an
 * identifier for primitive types like {@code int} and raw reference types like
 * {@code String} and {@code List}. It also identifies composite types like
 * {@code char[]} and {@code Set<Long>}.
 *
 * <p>
 * Type names are dumb identifiers only and do not model the values they name.
 * For example, the type name for {@code java.lang.List} doesn't know about the
 * {@code size()} method, the fact that lists are collections, or even that it
 * accepts a single type parameter.
 *
 * <p>
 * Instances of this class are immutable value objects that implement
 * {@code equals()} and {@code
 * hashCode()} properly.
 *
 ...
 * {@link #INT}, {@link #DOUBLE}, and {@link #VOID}.
 *
 * <p>
 * In an annotation processor you can get a type name instance for a type mirror
 * by calling {@link #get(TypeMirror)}. In reflection code, you can use
 * {@link #get(Type)}.
 *
 * <h3>Defining new types</h3>
 *
 * <p>
 * Create new reference types like {@code com.example.HelloWorld} with
 * {@link ClassName#get(String, String, String...)}. To build composite types
 * like {@code char[]} and {@code Set<Long>}, use the factory methods on
 * {@link ArrayTypeName}, {@link ParameterizedTypeName},
 * {@link TypeVariableName}, and {@link WildcardTypeName}.
 */
public class TypeName {
    ...
    /**
     * Returns true if this is a primitive type like {@code int}. Returns false
     * for all other types types including boxed primitives and {@code void}.
     */
     ...
     * Returns false for all other types types including unboxed primitives and
     * {@code java.lang.Void}.
     */
    public boolean isBoxedPrimitive() {
        return this.equals(BOXED_BOOLEAN) || this.equals(BOXED_BYTE) || this.equals(BOXED_SHORT)
                || this.equals(BOXED_INT) || this.equals(BOXED_LONG) || this.equals(BOXED_CHAR)
                || this.equals(BOXED_FLOAT) || this.equals(BOXED_DOUBLE);
    }

    /**
     * Returns a boxed type if this is a primitive type (like {@code Integer}
        ...
        return mirror.accept(new SimpleTypeVisitor7<TypeName, Void>() {
            ...
            public TypeName visitDeclared(DeclaredType t, Void p) {
                ...
                TypeName enclosing = (enclosingType.getKind() != TypeKind.NONE)
                        && !t.asElement().getModifiers().contains(Modifier.STATIC) ? enclosingType.accept(this, null)
                                : null;
    ...
    /**
     * Returns the array component of {@code type}, or null if {@code type} is
     * not an array.
     */
