...
/**
 * Assigns Java identifier names to avoid collisions, keywords, and invalid
 * characters. To use, first create an instance and allocate all of the names
 * that you need. Typically this is a mix of user-supplied names and constants:
 * 
 * <pre>
 * 
 * {
 ...
 * </pre>
 *
 * Pass a unique tag object to each allocation. The tag scopes the name, and can
 * be used to look up the allocated name later. Typically the tag is the object
 * that is being named. In the above example we use {@code property} for the
 * user-supplied property names, and {@code "string
 ...
 * </pre>
 *
 * The above code generates unique names if presented with conflicts. Given
 * user-supplied properties with names {@code ab} and {@code sb} this generates
 * the following:
 * 
 * <pre>
 *    {@code
 ...
 * </pre>
 *
 * The underscore is appended to {@code sb} to avoid conflicting with the
 ...
 *
 * <p>
 * When dealing with multiple independent inner scopes, use a {@link #clone()}
 * of the NameAllocator used for the outer scope to further refine name
 * allocation for a specific inner scope.
 */
public final class NameAllocator implements Cloneable {
    ...
    /**
     * Return a new name using {@code suggestion} that will not be a Java
     * identifier or clash with other names.
     */
    ...
    /**
     * Return a new name using {@code suggestion} that will not be a Java
     * identifier or clash with other names. The returned value can be queried
     * multiple times by passing {@code tag} to {@link #get(Object)}.
     */
    ...
    /**
     * Create a deep copy of this NameAllocator. Useful to create multiple
     * independent refinements of a NameAllocator to be used in the respective
     * definition of multiples, independently-scoped, inner code blocks.
     *
