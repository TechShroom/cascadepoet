...
public final class ParameterizedTypeName extends TypeName {
    ...
    private ParameterizedTypeName(ParameterizedTypeName enclosingType, ClassName rawType, List<TypeName> typeArguments,
        ...
        for (TypeName typeArgument : this.typeArguments) {
            checkArgument(!typeArgument.isPrimitive() && typeArgument != VOID, "invalid type parameter: %s",
                    typeArgument);
        }
    }
    ...
    public ParameterizedTypeName annotated(List<AnnotationSpec> annotations) {
        return new ParameterizedTypeName(this.enclosingType, this.rawType, this.typeArguments,
    ...
    CodeWriter emit(CodeWriter out) throws IOException {
        if (this.enclosingType != null) {
            this.enclosingType.emitAnnotations(out);
            this.enclosingType.emit(out);
            out.emit("." + this.rawType.simpleName());
        } else {
            this.rawType.emitAnnotations(out);
    ...
    /**
     * Returns a new {@link ParameterizedTypeName} instance for the specified
     * {@code name} as nested inside this class.
     */
    ...
    /**
     * Returns a new {@link ParameterizedTypeName} instance for the specified
     * {@code name} as nested inside this class, with the specified
     * {@code typeArguments}.
     */
    ...
    /**
     * Returns a parameterized type, applying {@code typeArguments} to
     * {@code rawType}.
     */
    ...
    static ParameterizedTypeName get(ParameterizedType type, Map<Type, TypeVariableName> map) {
        ...
        return (ownerType != null) ? get(ownerType, map).nestedClass(rawType.simpleName(), typeArguments)
